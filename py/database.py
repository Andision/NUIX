...
TABLE_CONF = {
    'sys_paired_device': '(device_id primary key, init_time, init_random_int, init_verified, transfer_data_key,transfer_data_key_expire_time )',
...
def sys_add_paired_device(device_id, init_time, init_random_int, init_verified):
    con, cur = open_table('sys_paired_device')
    cur.execute(
        'DELETE FROM sys_paired_device WHERE device_id="{}"'.format(device_id))
    cur.execute('INSERT INTO sys_paired_device(device_id, init_time, init_random_int, init_verified) values(?,?,?,?)',
                (device_id, init_time, init_random_int, init_verified))
    con.commit()


def sys_get_paired_device(device_id):
    con, cur = open_table('sys_paired_device')
    # cur.execute('SELECT device_id, init_time, init_random_int, init_verified FROM sys_paired_device ')
    cur.execute(
        'SELECT device_id, init_time, init_random_int, init_verified FROM sys_paired_device WHERE device_id="{}"'.format(device_id))
